...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Starting file: sim_and_real_compare.py')
        ...
        if logger_params['use_logger']:
            ...
            log_dir = os.path.join(rel_dirname, "simulator_" + logger_params['log_dir'])
            sim_logger = Logger(log_dir=log_dir, use_video=logger_params['use_video'])
        ...
        for trial_idx in range(10):  # TODO: num of trials param?
            ...
            sim_obs = sim_env.reset(shape_keys=['CylinderM'], hand_orientation='normal')  # TODO: make sure this is NO OBJECT
            sim_env.render()
            ...
            for timestep_idx in range(max_timesteps):  # or turn this into for loop
                ...
                sim_action = lerp(action, old_min=-6800, old_max=6800,
                                     new_min=-3,
                                     new_max=3)

                next_obs, reward, done, info = env.step(action)

                sim_next_obs, sim_reward, sim_done, sim_info = sim_env.step(np.concatenate((np.array([0]), sim_action)))
                sim_env.render()

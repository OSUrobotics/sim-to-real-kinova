...
class Actor(nn.Module):
    ...
    def forward(self, state):
        # print('weight shape: ', self.l1.weight.shape)
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        # print('burh', str(state_dim + action_dim))
    ...
    def forward(self, state, action):
        # combined_state_action = torch.cat([state, action], -1)
        # # print("pain")
        # # print(combined_state_action.shape)
...
class DDPGfD(object):
    def __init__(self, state_dim=82, action_dim=3, max_action=1.5, n=5, discount=0.995, tau=0.0005, batch_size=64,
    ...
    def select_action(self, state):
        ...
        # return self.actor(torch.zeros((54884883,)).cuda()).cpu().data.numpy().flatten()
    ...
    def train_batch_state_already_reduced(self, episode_num, update_count, expert_replay_buffer, replay_buffer):
        """ Update policy networks based on batch_size of episodes using n-step returns """
        self.total_it += 1
        agent_batch_size = 0
        expert_batch_size = 0
        ...
        if replay_buffer is not None and expert_replay_buffer is None:  # Only use agent replay
            ...
            expert_state, expert_action, expert_next_state, expert_reward, expert_not_done = expert_replay_buffer.sample_batch_nstep(
                expert_batch_size)
            state, action, next_state, reward, not_done = expert_state, expert_action, expert_next_state, expert_reward, expert_not_done

            # TODO: figure out if we need to set a new set of gradients for copying SARSA, or if we're fine using the same gradient tracking.
        else:
            # print("MIX OF AGENT AND EXPERT")
